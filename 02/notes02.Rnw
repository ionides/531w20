...
\begin{frame}[fragile]
\frametitle{Chapter \CHAPTER. Time series models, trend and autocovariance}
...
\begin{enumerate}
\item Set up general notation for working with time series data and time series models.

\item Define the trend function for a time series model, and discuss its estimation from data. 

\item Discuss the properties of least square estimation of the trend.

\item Define the autocovariance and autocorrelation functions and their standard estimators.
\end{enumerate}
\end{frame}
...
\item We keep $t$ to represent continuous time, and $n$ to represent the discrete sequence of observations. This will serve us well later, when we fit continuous time models to data.
...
\begin{frame}
\bi
\item Our notation for densities generalizes. We write $f_Y(y)$ for the density of a random variable $Y$ evaluated at $y$, and $f_{YZ}(y,z)$ for the joint density of the pair of random variables $(Y,Z)$ evaluated at $(y,z)$. We can also write $f_{Y|Z}(y|z)$ for the conditional density of $Y$ given $Z$.
...
\item Scientifically, we postulate that $\data{y_{1:N}}$ are a realization of $Y_{1:N}$ for some unknown value of $\theta$.
...
\item We write $\hat\mu_n(y_{1:N})$ to be some estimator of $\mu_n$, i.e., a map which is applied to the data to give an estimate of $\mu_n$.  An appropriate choice of $\hat\mu_n$ will depend on the data and the model.

\item Usually, applied statistics courses do not distinguish between estimators (functions that can be applied to any dataset) and estimates (an estimator evaluated on the actual data). For thinking about model specification and diagnosing model misspecification it is helpful to bear this in mind.

\item For example, the estimate of the mean function is the value of the estimator when applied to the data. Here, we write this as
$$ \data{\hat\mu_n} = \hat\mu_n(\data{y_{1:N}}).$$

\item We call $\data{\hat\mu_n}$ an \myemph{estimated mean function} or \myemph{estimated trend}.
...
\begin{frame}
\bi
\item  For example, sometimes we suppose a model with $\mu_n=\mu$, so the mean is assumed constant. In this case, the model is called \myemph{mean stationary}. Then, we might estimate $\mu$ using the mean estimator,
$$\hat\mu(y_{1:N})=\frac{1}{N}\sum_{k=1}^N y_k.$$

\item We write $\data{\hat\mu}$ for $\hat\mu(\data{y_{1:N}})$, the sample mean.

\item We can compute the sample mean, $\data{\hat\mu}$, for any dataset. It is only a reasonable estimator of the mean function when a mean stationary model is appropriate.

\item Notice that trend is a property of the model, and the estimated trend is a function of the data.

\item Formally, we should not talk about the trend of the data. People do, but we should try not to.
...
This is  called the \myemph{autocovariance} function, viewed as a function of $m$ and $n$.

\item We may also write $\Gamma$ for the matrix whose $(m,n)$ entry is $\gamma_{m,n}$.
...
\item For a covariance stationary model, and some mean function estimator $\hat\mu_n=\hat \mu_n(y_{1:N})$, a common estimator for $\gamma_h$ is
$$ \hat\gamma_h(y_{1:N}) = \frac{1}{N}\sum_{n=1}^{N-h} \big( {y_n} - \hat\mu_n \big)\, \big({y_{n+h}}-\hat\mu_{n+h} \big).$$

\item The corresponding estimate of $\gamma_h$, known as the \myemph{sample autocovariance function},
is
...
\item Here is a quote from {\it Science} (18 December 2015, volume 350, page 1461; I've added some emphasis).
...
\begin{frame}[fragile]
We write the data $\data{y_{1:N}}$ as a column vector,
\begin{eqnarray*}
...
\bi
\item The variance/covariance matrix of $\hat\beta_{OLS}(Y_{1:N})$ under this model is
...
\begin{frame}
\frametitle{Context for Homework 1}
...
\begin{frame}[fragile]
\bi
\item  Homework 1 asks you to relate this source code to the task of testing for lack of correlation, a standard topic in undergrad introductory statistics courses. The critical line of code seems to be
<<,eval=F,echo=T>>=
