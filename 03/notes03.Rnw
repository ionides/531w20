...
\myquestion. Question: Is it usual for time series to be well modeled as stationary (either weakly or strictly)?
...
\myquestion. Question: If data do not often show stationary behavior, why do many fundamental models have stationarity?
...
\myquestion. Question: Is a stationary model appropriate for either (or both) the time series below? Explain.
...
{\myquestion} \myemph{A}. Show that $\epsilon_{1:N}$ is weakly stationary, and is white noise!
...
{\myquestion} \myemph{B}. Show that $\epsilon_{1:N}$ is NOT strictly stationary.
...
\frametitle{Using white noise as a building block for other time series models}
...
\item Looking at simulated sample paths is a good way to get some intuition about a random process model. 

\item We will do this for the AR(1) model M2

\item One approach is to use the \code{arima.sim} function in R.
...
\item A direct approach to simulating model M2 is to write out the model equation explicitly.
...
\item This looks very similar to the \code{arima.sim} simulation, except for a difference near the start. Can you explain this? Hint: How does \code{arima.sim} initialize the simulation?
...
\myquestion. Exercise: Compute the autcovariance function for model M2.
...
\item Let's simulate M4 using both the methods we tried for the autoregressive model
...
\item \code{X1} and \code{X2} look identical. We can check this
<< check>>=
all(X1==X2)
@

\item Perhaps you agree that the spurious evidence for a trend that we saw for the AR(1) model is still somewhat present for the MA(2) simulation.

\item We could be curious about what the underlying white noise process looks like
...
\item To me, the trend-like behavior is not visually apparent in the white noise that ``drives'' the AR and MA models.
...
\frametitle{A random walk model}
...
\myquestion. Exercise: compute the mean and covariance functions for the random walk model with and without drift. 
...
\frametitle{An example of a random walk: Modeling financial markets}

\bi
\item The theory of efficient financial markets suggests that the logarithm of a stock market index (or, for that matter, the value of an individual stock or other investment) might behave like a random walk with drift

\item Let's test that out on daily S\&P 500 data, downloaded from \url{yahoo.com}.
...
\begin{frame}[fragile]
\bi
\item To train our intuition, we can compare the data with simulations from a fitted model. A simple starting point is a Gaussian random walk with drift, having parameters estimated by
...
\item Seems reasonable enough so far. Let's plot the sample autocorrelation function (sample ACF) of \code{diff(log(sp500))}. 
...
\item The temporal difference of the log of the value of an investment is called the \myemph{return} on the investment. Let's plot the sample autocorrelation function of the time series of S\&P 500 returns, $\data{z_{2:N}}$.
<< sp500_acf>>=
z <- diff(log(sp500))
acf(z)
@
