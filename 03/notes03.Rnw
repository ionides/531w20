...
\answer{\vspace{50mm}}{If $\epsilon_1$ is close to 1 then $\epsilon_2$ is necessarily close to 0. This violated an independence relation: $\prob[1-\delta\le \epsilon_1\le 1,0\le \epsilon_2\le \delta]\neq \prob[1-\delta\le \epsilon_1\le 1]\,\prob[0\le \epsilon_2\le \delta]$.}
