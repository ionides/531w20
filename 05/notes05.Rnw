...
$\estimate{\hat\theta_d} \pm 1.96 \big[{\estimate{\hat{I}}}^{-1}\big]_{dd}^{1/2}$. The R function \code{arima} computes standard errors for the MLE of an ARMA model in this way.
...
3. For large $J$, the coverage of the proposed confidence interval is well approximated, for models in a neighborhood of $\estimate{\hat\theta}$, by the proportion of the intervals $\big[\theta^{[j]}_{1,\mathrm lo},\theta^{[j]}_{1,\mathrm hi}\big]$ that include $\estimate{\hat\theta_1}$.

\vspace{1mm}

4. The sample standard deviation of $\{ \theta^{[j]}_1, j\in 1:J\}$ is a natural standard error to associate with $\estimate{\hat \theta_1}$.
...
\frametitle{Likelihood-based inference for ARMA models in R}
...
\myquestion. When do you think the Fisher information confidence interval may be reliable?
...
\item Numerical optimization is more problematic. 
 ...
 \item Evidence for imperfect maximization (assuming negligible likelihood evaluation error) can be found in the above AIC table.
 \ei
