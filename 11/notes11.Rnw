...
<<sir-diagram,echo=FALSE,cache=TRUE,purl=FALSE>>=
require(DiagrammeR)
...
$$\mu_{SI}(t)=\beta \, I(t),$$
with $\beta$ being the \myemph{contact rate}. Now, for the simplest SIR model, ignoring demography, we set
...
\item Here, we are using [little $o$ notation]
We write 
$$ h(\delta)=o(\delta)$$
to mean 
