...
\item One major applications of compartment models is pharmacokinetics,
%% (https://en.wikipedia.org/wiki/Pharmacokinetics), 
the study of how pharmacological drugs enter the body, move between organs, metabolize, and leave. The compartments may be the organs; the flow is movement of the drug and its metabolites between organs.

\item Another major application of compartment models is epidemiology.
%% (https://en.wikipedia.org/wiki/Compartmental_models_in_epidemiology). 
Compartments are groups of people; the flow
...
A starting point is to construct \code{rprocess}, a stochastic simulator for the unobserved state process.
We saw several numerical approaches to do this.
...
\begin{frame}[fragile]
A good first step is to perform some simulations to check whether things are working.
To do so, we need to choose parameters.
...
\frametitle{Extending the SIR model to include latency: The SEIR model}
...
As described in the report \citep{anonymous78}, $B_t$ represents the total number of boys \myemph{in} bed on day $t$.
