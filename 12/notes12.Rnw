...
\bi
\item  To develop and debug code, we want a version that runs extra quickly, for which we set \code{run_level=1}.
...
\item For more precise computations, a more intensive version is \code{run_level=3}.
\ei
<<run_level>>=
run_level <- 1
switch(run_level, {
