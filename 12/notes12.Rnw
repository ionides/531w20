...
\frametitle{Likelihood maximization}
\bi
\item  We saw above that the default parameter set for the 'bsflu' pomp object is not particularly close to the MLE.

\item  One approach to find the MLE is to apply an optimizer to the particle filter estimate of the likelihood.
...
\item  We use the IF2 algorithm of \citet{ionides15}.
...
\bi
\item  The starting parameter swarm, $\{ \Theta^0_j, j=1,\dots,J\}$, usually consists of $J$ identical replications of some starting parameter vector.
...
\item  $R_1$ counts individuals who are bed-confined if they show symptoms; $R_2$ counts individuals who are convalescent if they showed symptoms; $R_3$ counts recovered individuals who have returned to schoolwork if they were symtomatic.  

\item We use abbreviations $\mu_{IR}=\mu_{IR_1}$, $\mu_{R_1}=\mu_{R_1R_2}$, $\mu_{R_2}=\mu_{R_2R_3}$.
...
\item  In  \Sexpr{round(t_pf["elapsed"],1)} seconds, we obtain an unbiased likelihood estimate of \Sexpr{round(L_pf[1],2)} with a Monte~Carlo standard error of \Sexpr{round(L_pf[2],2)}.
