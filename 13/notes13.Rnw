...
\item Here, a natural starting point is the MLE reported by \citet{martinez-bakker15}.
\ei
...
\item In  \Sexpr{round(t1["elapsed"],1)} seconds, we obtain an unbiased likelihood estimate of \Sexpr{round(L1[1],2)} with a Monte standard error of \Sexpr{round(L1[2],2)}.
...
\item
The MLE reported for these data by \citet{martinez-bakker15} is -794.34, with Monte Carlo evaluation error of 0.18.
\item This is similar to the log likelihood at their MLE for our model (\Sexpr{round(L1[1],2)} with Monte Carlo evaluation error \Sexpr{round(L1[2],2)}).
