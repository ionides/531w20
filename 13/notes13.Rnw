...
% \item An example of identifying lag in the midterm project: [The relationship between S&P500 and CPI] \url{https://ionides.github.io/531w16/midterm_project/project20/midterm.html#try-other-lags}
...
\item This is called the \myemph{cross-correlation function} and can be computed with the R function \code{ccf}.
 ...
 \item An example of the use of the cross-correlation function in a midterm project: [The Association between Recent Cholera Epidemics and Rainfall in Haiti](https://ionides.github.io/531w16/midterm_project/project3/midterm_project.html).
...
\item Time series modeling of regression errors is only one of many ways in which covariates could be used to explain a dynamic system.

\item In general, it is nice if scientific considerations allow you to propose sensible ways to model the relationship.
...
\item All that remains is to hypothesize what is a reasonable way to include covariates for your system, and to implement the resulting data analysis.

\item The \package{pomp} package provides facilities for including covariates in a pomp object, and making sure that the covariates are accessible to \code{rprocess}, \code{dprocess}, \code{rmeasure}, \code{dmeasure}, and  the state initialization at time $t_0$.

\item Named covariate time series entered via the \code{covar} argument to \code{pomp} are automatically defined within Csnippets used for the \code{rprocess}, \code{dprocess}, \code{rmeasure}, \code{dmeasure} and \code{initializer} arguments.

\item Let's see this in practice, using population census, birth data and seasonality as covariates in an epidemiological model.
...
\item We implement the compartment model of \citep{martinez-bakker15}, having compartments representing susceptible babies in each of six one-month birth cohorts ($S^B_1$,...,$S^B_6$), susceptible older individuals ($S^O$), infected babies ($I^B$), infected older individuals ($I^O$), and recovered with lifelong immunity ($R$). 
...
\item Babies under six months are modeled as fully protected from symptomatic poliomyelitis; older infections lead to reported cases (usually paralysis) at a rate $\rho$. 

\item The flows through the compartments are graphically represented as follows (Figure 1A of \citep{martinez-bakker15}):
...
\item Since duration of infection is comparable to the one-month reporting aggregation, a discrete time model may be appropriate. \citep{martinez-bakker15} fitted monthly observations from May 1932 through January 1953, so we define $t_n=1932+ (4+n)/12$ for $n=0,\dots,N$, and we write
...
\item We employ a continuous population model, with no demographic stochasticity (in some sense, the demographic-scale stochasticity in $\lambda_n$ is in fact environmental stochasticity since it modifies a rate that affects all compartments equally). Writing $B_n$ for births in month $n$, we obtain the dynamic model of \citep{martinez-bakker15}:
$$\begin{array}{rcl}
S^B_{1,n+1}&=&B_{n+1}\\
...
Following \citep{martinez-bakker15}, we make an approximation for the initial conditions of ignoring infant infections at time $t_0$. 
...
\frametitle{Building a pomp object for the polio model}
...
Now, let's define the covariates. \code{time} gives the time at which the covariates are defined. \code{P} is a smoothed interpolation of the annual census. \code{B} is monthly births. The B-spline basis is coded as \code{xi1,...,xi6} 
...
The _initial value parameters_ (IVPs), $\tilde I^O_{0}$ and  $\tilde S^O_{0}$, are coded for each state named by adding \code{_0} to the state name:
...
Alternatively, these fixed quantities could be passed as constants using the \code{globals} argument of \code{pomp}. We can check how the initial birth parameters are set up:
...
We read in a table of previous parameter search results from \code{polio_params.csv}, and take the one with highest likelihood as our current estimate of an MLE. We can inspect that the fixed parameters are indeed set to their proper values.
...
\item To develop and debug code, it is nice to have a version that runs extra quickly.

\item To facilitate switching quickly between versions of the document that have different run times, we set up \code{run_level} options. 

\item \code{run_level=1} will set all the algorithmic parameters to the first column of values in the following code.

\item Here, \code{Np} is the number of particles (i.e., sequential Monte Carlo sample size), and \code{Nmif} is the number of iterations of the optimization procedure carried out below. 

\item Empirically, \code{Np=5000} and \code{Nmif=200} are around the minimum required to get stable results with an error in the likelihood of order 1 log unit for this example; this is implemented by setting \code{run_level=2}. 

\item One can then ramp up to larger values for more refined computations, implemented here by \code{run_level=3}.

\ei

<<run_level>>=
run_level=1
polio_Np <-          switch(run_level,100, 5e3, 1e4)
polio_Nmif <-        switch(run_level, 10, 200, 400)
...
\item Here, \code{run_level} is coded differently from the previous case studies. It is functionally equivalent. Which way you prefer is up to you, but seeing different alternatives may help to clarify the goal of the code.
...
\myquestion. Choosing algorithmic parameters.\\
...
\frametitle{Likelihood evaluation at an estimated MLE}

\bi

\item Let's carry out a likelihood evaluation at the reported MLE.  

\item Since most modern machines have multiple cores, it is convenient to do some parallelization to generate replicated calls to \code{pfilter}. Notice that the replications are averaged using the \code{logmeanexp} function.
...
\item In  \Sexpr{round(t1[""],1)\code{ seconds, we obtain an unbiased likelihood estimate of \Sexpr{round(L1[1],2)} with a Monte standard error of \Sexpr{round(L1[2],2)}.
...
This setup has minor differences in notation, model construction and code compared to \citep{martinez-bakker15}. The MLE reported for these data by \citep{martinez-bakker15} is -794.34 (with Monte Carlo evaluation error of 0.18) which is similar to the log likelihood at the MLE for our model (\Sexpr{round(L1[1],2)} with Monte Carlo evaluation error \Sexpr{round(L1[2],2)}). This suggests that the differences do not substantially improve or decrease the fit of our model compared to  \citep{martinez-bakker15}. When different calculations match reasonably closely, it demonstrates some reproducibility of both results.

\end{frame}

\begin{frame}[fragile]

\frametitle{Simulation to investigate the fitted model: Local persistence}

The scientific purpose of fitting a model typically involves analyzing properties of the fitted model, often investigated using simulation. Following \citep{martinez-bakker15}, we are interested in how often months with no reported cases ($Y_n=0$) correspond to months without any local asymptomatic cases, defined for our continuous state model as $I^B_n+I^O_n<1/2$. For Wisconsin, using our model at the estimated MLE, we compute as follows:
...
\item Let's see if we can improve on the previous MLE. We use the iterated filtering algorithm IF2 of @ionides15, which uses a random walk in parameter space to approach the MLE. We set a constant random walk standard deviation for each of the regular parameters and a larger constant for each of the initial value parameters. 
...
\item This investigation took  \Sexpr{round(t2["elapsed"]/60,1)} minutes. These repeated stochastic maximizations can also show us the geometry of the likelihood surface in a neighborhood of this point estimate:
...
\item Linear, Gaussian auto-regressive moving-average (ARMA) models provide non-mechansitic fits to the data including flexible dependence relationships. We fit to $\log(y_n^*+1)$ and correct the likelihood back to the scale appropriate for the untransformed data:
...
\item This 7-parameter model, which knows nothing of susceptible depletion, attains a likelihood of \Sexpr{round( arma_fit$loglik-sum(log_y),1)}. Although our goal is not to beat non-mechanstic models, it is comforting that we're competitive with them.
...
\item Saving the results of previous searches, with likelihoods that have been repeatedly evaluated by particle filters, gives a resource for building up knowledge about the likelihood surface. Above, we have added our new results to the file \code{polio_params.csv}, which we now investigate.
...
\item Here, we see that the most successful searches have always led to models with reporting rate around 1%. This impression can be reinforced by looking at results from the global searches:
...
\item We see that reporting rates close to 1% seem to provide a small but clear (several units of log likelihood) advantage in explaining the data. These are the reporting rates for which depletion of susceptibles can help to explain the dynamics.
...
\myquestion. Parameter estimation using randomized starting values.
...
One could look at filtering diagnostics at the MLE, for example, \code{plot(pf1[[1]])} but the diagnostic plots for iterated filtering include filtering diagnostics for the last iteration anyhow, so let's just consider the \code{mif} diagnostic plot. Looking at several simultaneously permits assessment of Monte Carlo variability. \code{plot} applied to a \code{mifList} object does this: here, \code{m3} is of class \code{mifList} since that is the class resulting from concatenation of \code{mif2d.pomp} objects using \code{c()}:
